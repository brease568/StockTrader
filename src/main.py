...
class StockScanner:
  ...
  def generate_data(self, scan_back_to_date, username):
    ...
    while True:
        ...
        #page = requests.get("https://profit.ly/user/timothysykes/trades?page=1&size=10")
        ...
        for _ in range(2):
            ...
            for card_header in all_card_headers:
                ...
                #if trade_formatted_date > datetime.datetime(2021, 8, 9, 13, 10):
                if trade_formatted_date > scan_back_to_date:
                    print("Trade formatted date is after the 9th @ 13:10")
                else:
                    print("Trade formatted date is on or before the 9th @ 13:10")
                    break
                ...
                print(f"Entry: {trade_entry}")
                print(f"Exit: {trade_exit}")
                ...
                print(f"Position Size: {trade_position_size}")
                
                trade_percentage_profit = trade_position_size_table[2].span.get_text().strip()
                print(f"Percent Profit: {trade_percentage_profit}\n")
  ...
  def perform_stock_analysis(self, stock_list):
    for stock in stock_list:
      ...
      #stock_analyzer = StockAnalyzer(stock.ticker)
      stock_analyzer.analyze()
      
      #stock.float = stock_analyzer.float
      #stock.shares_outstanding = stock_analyzer.shares_outstanding
      #stock.implied_shares_outstanding = stock_analyzer.implied_shares_outstanding

      #stock.todays_high = stock_analyzer.todays_high
      #stock.yesterdays_high = stock_analyzer.yesterdays_high
      #stock.minus_two_days_high = stock_analyzer.minus_two_days_high
      #stock.minus_three_days_high = stock_analyzer.minus_three_days_high

      #stock.todays_low = stock_analyzer.todays_low
      #stock.yesterdays_low = stock_analyzer.yesterdays_low
      #stock.minus_two_days_low = stock_analyzer.minus_two_days_low
      #stock.minus_three_days_low = stock_analyzer.minus_three_days_low

      #stock.todays_volume = stock_analyzer.todays_volume
      #stock.yesterdays_volume = stock_analyzer.yesterdays_volume
      #stock.minus_two_days_volume = stock_analyzer.minus_two_days_volume
      #stock.minus_three_days_volume = stock_analyzer.minus_three_days_volume

      #stock.price_percent_change_today = stock_analyzer.price_percent_change_today
      #stock.price_percent_change_since_yesterday = stock_analyzer.price_percent_change_since_yesterday
      #stock.price_percent_change_since_two_days = stock_analyzer.price_percent_change_since_two_days

      #stock.volume_percent_change_since_yesterday = stock_analyzer.volume_percent_change_since_yesterday
      #stock.volume_percent_change_since_two_days = stock_analyzer.volume_percent_change_since_two_days
...
def create_new_stock_data(stock_scan, username):
  ...
  # Need to do something here about passing the most recent date - essentially fake date
